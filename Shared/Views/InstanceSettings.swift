...
struct InstanceSettings: View {
    ...
    var body: some View {
        Form {
            ...
            Section(header: Text("Scramble"), footer: Text(instance.showScramble && Algorithm.scramble(puzzle: instance.puzzle) == nil ? "If a puzzle doesn't have a build in scramble generator, you can create your own using Javascript." : "")) {
        ...
        .sheet(isPresented: $showingCodeEditorSheet) {
            JavaScriptEditor("\(instance.name) - Scramble", code: $instance.customScrambleAlgorithm)
        }
    }
}
